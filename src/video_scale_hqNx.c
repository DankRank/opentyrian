...
#define VGA2RGB(p) rgb_palette[p]
#define VGA2YUV(p) yuv_palette[p]
...
/* (c1*w1 + c2*w2) >> s */
static always_inline uint32_t interp_2px(uint8_t p1, int w1, uint8_t p2, int w2, int s)
{
	uint32_t c1 = VGA2RGB(p1),
	         c2 = VGA2RGB(p2);
...
/* (c1*w1 + c2*w2 + c3*w3) >> s */
static always_inline uint32_t interp_3px(uint8_t p1, int w1, uint8_t p2, int w2, uint8_t p3, int w3, int s)
{
	uint32_t c1 = VGA2RGB(p1),
	         c2 = VGA2RGB(p2),
	         c3 = VGA2RGB(p3);
...
#define WDIFF(c1, c2) yuv_diff(VGA2YUV(c1), VGA2YUV(c2))
...
static always_inline void hqx_filter(SDL_Surface *src_surface, SDL_Texture *dst_texture, int n)
{
	...
	for (y = 0; y < height; y++) {
		...
		for (x = 0; x < width; x++) {
			...
			const uint32_t yuv1 = VGA2YUV(w[4]);
			const int pattern = (w[4] != w[0] ? (yuv_diff(yuv1, VGA2YUV(w[0]))) : 0)
			                  | (w[4] != w[1] ? (yuv_diff(yuv1, VGA2YUV(w[1]))) : 0) << 1
			                  | (w[4] != w[2] ? (yuv_diff(yuv1, VGA2YUV(w[2]))) : 0) << 2
			                  | (w[4] != w[3] ? (yuv_diff(yuv1, VGA2YUV(w[3]))) : 0) << 3
			                  | (w[4] != w[5] ? (yuv_diff(yuv1, VGA2YUV(w[5]))) : 0) << 4
			                  | (w[4] != w[6] ? (yuv_diff(yuv1, VGA2YUV(w[6]))) : 0) << 5
			                  | (w[4] != w[7] ? (yuv_diff(yuv1, VGA2YUV(w[7]))) : 0) << 6
			                  | (w[4] != w[8] ? (yuv_diff(yuv1, VGA2YUV(w[8]))) : 0) << 7;
			...
			if (n == 2) {
				...
				dst32[dst32_linesize + 1] = VGA2RGB(w[4]);                                                                        // 11
			}
			else if (n == 4) {
