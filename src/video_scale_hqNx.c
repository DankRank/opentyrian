 ...
 * 
 * Original hq2x, hq3x, hq4x
 * Copyright (C) 2003 MaxSt ( maxst@hiend3d.com )
 *
 * ffmpeg implementation (libavfilter/vf_hqx.c)
...
static always_inline int yuv_diff(uint32_t yuv1, uint32_t yuv2)
{
...
/* (c1*w1 + c2*w2) >> s */
static always_inline uint32_t interp_2px(uint32_t c1, int w1, uint32_t c2, int w2, int s)
{
...
/* (c1*w1 + c2*w2 + c3*w3) >> s */
static always_inline uint32_t interp_3px(uint32_t c1, int w1, uint32_t c2, int w2, uint32_t c3, int w3, int s)
{
...
#define YDIFF(c1, c2) yuv_diff(c1, c2)
...
static always_inline uint32_t hq2x_interp_1x1(int k,
                                              ...
                                              int p6, int p7, int p8)
{
	...
	if ((P(0xbf, 0x37) || P(0xdb, 0x13)) && YDIFF(v1, v5))
		return interp_2px(w4, 3, w3, 1, 2);
	if ((P(0xdb, 0x49) || P(0xef, 0x6d)) && YDIFF(v7, v3))
		return interp_2px(w4, 3, w1, 1, 2);
	if ((P(0x0b, 0x0b) || P(0xfe, 0x4a) || P(0xfe, 0x1a)) && YDIFF(v3, v1))
		return w4;
...
static always_inline void hq3x_interp_2x1(uint32_t *dst, int dst_linesize,
                                          int k,
                                          const uint32_t *w, const uint32_t *v,
                                          int pos00, int pos01,
                                          int p0, int p1, int p2,
                                          ...
                                          int rotate)
{
	...
	else if ((P(0xbf, 0x37) || P(0xdb, 0x13)) && YDIFF(v1, v5))
		*dst00 = interp_2px(w4, 3, w3, 1, 2);
	else if ((P(0x0b, 0x0b) || P(0xfe, 0x4a) || P(0xfe, 0x1a)) && YDIFF(v3, v1))
		*dst00 = w4;
	...
	if ((P(0xfe, 0xde) || P(0x9e, 0x16) || P(0xda, 0x12) || P(0x17, 0x16) ||
	     P(0x5b, 0x12) || P(0xbb, 0x12)) && YDIFF(v1, v5))
		*dst01 = w4;
	else if ((P(0x0f, 0x0b) || P(0x5e, 0x0a) || P(0xfb, 0x7b) || P(0x3b, 0x0b) ||
	          P(0xbe, 0x0a) || P(0x7a, 0x0a)) && YDIFF(v3, v1))
		*dst01 = w4;
 ...
 * pixels. */
static always_inline void hq4x_interp_2x2(uint32_t *dst, int dst_linesize,
                                          int k,
                                          const uint32_t *w, const uint32_t *v,
                                          int pos00, int pos01,
                                          int pos10, int pos11,
                                          ...
                                          int p6, int p7, int p8)
{
	...
	const int cond00 = (P(0xbf, 0x37) || P(0xdb, 0x13)) && YDIFF(v1, v5);
	const int cond01 = (P(0xdb, 0x49) || P(0xef, 0x6d)) && YDIFF(v7, v3);
...
static always_inline void hqx_filter(SDL_Surface *src_surface, SDL_Texture *dst_texture, int n)
{
	int x, y;
	const int height = vga_height;
	const int width = vga_width;
	int dst_linesize;
	const int src_linesize = src_surface->pitch;
	uint8_t       *dst;
	const uint8_t *src = src_surface->pixels;

	SDL_LockTexture(dst_texture, NULL, &dst, &dst_linesize);
	...
	for (y = 0; y < height; y++) {
		...
		const int prevline = y > 0          ? -src_linesize : 0;
		const int nextline = y < height - 1 ?  src_linesize : 0;

		for (x = 0; x < width; x++) {
			const int prevcol = x > 0         ? -1 : 0;
			const int nextcol = x < width - 1 ?  1 : 0;
			const uint8_t vga[3 * 3] = {
				src8[prevcol + prevline], src8[prevline], src8[prevline + nextcol],
				src8[prevcol           ], src8[       0], src8[           nextcol],
				src8[prevcol + nextline], src8[nextline], src8[nextline + nextcol]
			};
			const uint32_t w[3 * 3] = {
				rgb_palette[vga[0]], rgb_palette[vga[1]], rgb_palette[vga[2]],
				rgb_palette[vga[3]], rgb_palette[vga[4]], rgb_palette[vga[5]],
				rgb_palette[vga[6]], rgb_palette[vga[7]], rgb_palette[vga[8]],
			};
			const uint32_t v[3 * 3] = {
				yuv_palette[vga[0]], yuv_palette[vga[1]], yuv_palette[vga[2]],
				yuv_palette[vga[3]], yuv_palette[vga[4]], yuv_palette[vga[5]],
				yuv_palette[vga[6]], yuv_palette[vga[7]], yuv_palette[vga[8]],
			};
			const int pattern = (w[4] != w[0] ? yuv_diff(v[4], v[0]) : 0)
			                  | (w[4] != w[1] ? yuv_diff(v[4], v[1]) : 0) << 1
			                  | (w[4] != w[2] ? yuv_diff(v[4], v[2]) : 0) << 2
			                  | (w[4] != w[3] ? yuv_diff(v[4], v[3]) : 0) << 3
			                  | (w[4] != w[5] ? yuv_diff(v[4], v[5]) : 0) << 4
			                  | (w[4] != w[6] ? yuv_diff(v[4], v[6]) : 0) << 5
			                  | (w[4] != w[7] ? yuv_diff(v[4], v[7]) : 0) << 6
			                  | (w[4] != w[8] ? yuv_diff(v[4], v[8]) : 0) << 7;

			if (n == 2) {
				dst32[dst32_linesize * 0 + 0] = hq2x_interp_1x1(pattern, w, v, 0, 1, 2, 3, 4, 5, 6, 7, 8);  // 00
				dst32[dst32_linesize * 0 + 1] = hq2x_interp_1x1(pattern, w, v, 2, 1, 0, 5, 4, 3, 8, 7, 6);  // 01 (vert mirrored)
				dst32[dst32_linesize * 1 + 0] = hq2x_interp_1x1(pattern, w, v, 6, 7, 8, 3, 4, 5, 0, 1, 2);  // 10 (horiz mirrored)
				dst32[dst32_linesize * 1 + 1] = hq2x_interp_1x1(pattern, w, v, 8, 7, 6, 5, 4, 3, 2, 1, 0);  // 11 (center mirrored)
			}
			else if (n == 3) {
				hq3x_interp_2x1(dst32,                          dst32_linesize, pattern, w, v, 0, 1, 0, 1, 2, 3, 4, 5, 6, 7, 8, 0);  // 00 01
				hq3x_interp_2x1(dst32 + 1,                      dst32_linesize, pattern, w, v, 1, 3, 2, 5, 8, 1, 4, 7, 0, 3, 6, 1);  // 02 12 (rotated to the right)
				hq3x_interp_2x1(dst32 + 1 * dst32_linesize,     dst32_linesize, pattern, w, v, 2, 0, 6, 3, 0, 7, 4, 1, 8, 5, 2, 1);  // 20 10 (rotated to the left)
				hq3x_interp_2x1(dst32 + 1 * dst32_linesize + 1, dst32_linesize, pattern, w, v, 3, 2, 8, 7, 6, 5, 4, 3, 2, 1, 0, 0);  // 22 21 (center mirrored)
				dst32[dst32_linesize + 1] = w[4];                                                                                    // 11
			}
			else if (n == 4) {
				hq4x_interp_2x2(dst32,                          dst32_linesize, pattern, w, v, 0, 1, 2, 3, 0, 1, 2, 3, 4, 5, 6, 7, 8); // 00 01 10 11
				hq4x_interp_2x2(dst32 + 2,                      dst32_linesize, pattern, w, v, 1, 0, 3, 2, 2, 1, 0, 5, 4, 3, 8, 7, 6); // 02 03 12 13 (vert mirrored)
				hq4x_interp_2x2(dst32 + 2 * dst32_linesize,     dst32_linesize, pattern, w, v, 2, 3, 0, 1, 6, 7, 8, 3, 4, 5, 0, 1, 2); // 20 21 30 31 (horiz mirrored)
				hq4x_interp_2x2(dst32 + 2 * dst32_linesize + 2, dst32_linesize, pattern, w, v, 3, 2, 1, 0, 8, 7, 6, 5, 4, 3, 2, 1, 0); // 22 23 32 33 (center mirrored)
			}
			else {
				assert(0);
			}
...
void hq2x_32(SDL_Surface *src_surface, SDL_Texture *dst_texture)
{
	hqx_filter(src_surface, dst_texture, 2);
}

void hq3x_32(SDL_Surface *src_surface, SDL_Texture *dst_texture)
{
	hqx_filter(src_surface, dst_texture, 3);
}

void hq4x_32(SDL_Surface *src_surface, SDL_Texture *dst_texture)
{
	hqx_filter(src_surface, dst_texture, 4);
}
